import { Engine, Scene, SpriteMap, Texture, Vector2, Vector3 } from "babylonjs";

export class QueldanasMap {
    public constructor(private scene: Scene) {}

    initialize() {
        // Load the JSON file, for simplicity in this demonstration it is included in-line.
    let atlasJSON = this.getTileset();
    
    // Load the SpriteSheet Associated with the JSON Atlas.
    let spriteSheet = new Texture('./assets/sprites/Legends_Level_A.png', this.scene,
        false, //NoMipMaps
        ...
        null, //CustomBuffer
        false, //DeleteBuffer
        Engine.TEXTURETYPE_FLOAT_32_UNSIGNED_INT_24_8_REV  //ImageFormageType RGBA TEXTURETYPE_RGBA
    );
    
    spriteSheet.wrapV = Texture.CLAMP_ADDRESSMODE;
    spriteSheet.wrapU = Texture.CLAMP_ADDRESSMODE; //Or Wrap, its up to you...
    
    let backgroundSize = new Vector2(200,60);
    
    let background = new SpriteMap('background', atlasJSON, spriteSheet,
    {
        ...
        colorMultiply : new Vector3(0.3,0.3,0.3)
    },
	this.scene);    
    ...
    for(var i = 0; i<background.spriteCount; i++){
        background.changeTiles(0, new Vector2(i + 1, backgroundSize.y - 1), i)
    }
    ...
    for(let x = 15; x < backgroundSize.x - 15; x++){
        for(let y = backgroundSize.y - 26; y > 0; y--){
            if(x % 12 == 0){
                tilePositions.push(new Vector2(x, y))
            }
        }
    }
    ...
    for(let x = 15; x < backgroundSize.x - 15; x++){
        if(x % 12 == 0){
            tilePositions.push(new Vector2(x, backgroundSize.y - 26))
        }
    }
    ...
    let levelSize = new Vector2(80,40);
    
    let levelBase = new SpriteMap('base', atlasJSON, spriteSheet,
    {
        stageSize: levelSize,
        maxAnimationFrames:8,
        baseTile : 42,
        layerCount: 2,
        flipU: true,
        colorMultiply : new Vector3(0.6,0.6,0.6)
    },
    this.scene);
    ...
    for(let x = 15; x < levelSize.x - 15; x++){
        if((x + 6) % 12 == 0){
            levelBase.changeTiles(1, new Vector2(x, 18),
    ...
    for(let x = 15; x < levelSize.x - 15; x++){
        if(x % 6 == 0){
         tilePositions.push(new Vector2(x, 16))
        }
    }
    ...
    for(let x = 15; x < levelSize.x - 15; x++){
        if(x % 6 == 0){
            tilePositions.push(new Vector2(x, 25))
        }
    }
    ...
    let levelStage = new SpriteMap('levelStage', atlasJSON, spriteSheet,
    {
        stageSize: levelSize,
        maxAnimationFrames:8,
        baseTile : 42,
    ...
    levelStage.loadTileMaps('./assets/maps/levelStage.tilemaps')
